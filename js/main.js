...
function ageClassification(age) {
  return age < 0 ? null : age <= 24 ? 'детский возраст' : age <= 44 ? 'молодой возраст' : age <= 65 ? 'средний возраст' : age <= 75 ? 'пожилой возраст' : age <= 90 ? 'старческий возраст' : age <= 122 ? 'долгожители' : null;
};
...
function oddFn(n) {
  let newArr = [];
  var i = 0;
  while (i++ < n) if (i % 2 !== 0) newArr.push(i);
